...
def check_hardsextube_video(o, url, host = None, path = None, query = None):
    matched, website_id, video_id, format, search, queue, report_hit = True, 'hardsextube', None, '', True, True, True

    if not (host and path and query):
        fragments = urlparse.urlsplit(url)
        [host, path, query] = [fragments[1], fragments[2], fragments[3]]

    if host.find('.hardsextube.com') > -1 and VALIDATE_HARDSEXTUBE_VIDEO_EXT_REGEX.search(path):
        try:
