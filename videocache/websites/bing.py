...
VALIDATE_BING_PATH_REGEX = re.compile('\/entityid\/[a-z0-9A-Z_\-]+')
...
def check_bing_video(o, url, host = None, path = None, query = None):
    ...
    if (host.find('msn.com') > -1 or VALIDATE_BING_DOMAIN_REGEX.search(host)) and VALIDATE_BING_VIDEO_EXT_REGEX.search(path):
        try:
            ...
            pass
    elif (host.find('.video.msn.com') > -1 and VALIDATE_BING_PATH_REGEX.search(path)):
        try:
            video_id = urllib.quote(path.strip('/').split('/')[-1]) + '.mp4'
        except Exception, e:
            pass
    else:
        matched = False
