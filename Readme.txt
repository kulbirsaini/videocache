Name and Description
    ...
    myspace, xhamster, xvideos, pornhub, spankwire, hardsextube, keezmovies, redtube, xtube, youporn, msn soapbox, tube8, blip.tv, break.com and wrzuta.pl
    videos.  It  can  cache videos from various websites in a separate directory (other than Squid cache), in a browsable fashion and can serve the subse-
    quent requests from the cache. It helps in saving bandwidth and reducing load time of the videos. Videocache is currently used by a number of ISPs  in
...
Squid Configuration
    Add the following lines to your squid.conf which is generally located at /etc/squid/squid.conf
...
Videocache Global Configuration
    ...
    max_logfile_backups, max_scheduler_logfile_backups, max_cleaner_logfile_backups, max_tracefile_backups
           The  logfiles are automatically rotated once they have exceeded the max_logfile_size. The max_logfile_backups is the number of backup files     want to keep.
